                            ...
                            html += '<div class="list-item" style="background: white; border: 1px solid #e0e0e0; border-radius: 12px; padding: 20px; margin-bottom: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); transition: all 0.3s ease; cursor: pointer;" onclick="showDonationDetail(' + donation.donationId + ')" onmouseenter="this.style.boxShadow=\'0 4px 16px rgba(0,0,0,0.12)\'; this.style.transform=\'translateY(-2px)\'" onmouseleave="this.style.boxShadow=\'0 2px 8px rgba(0,0,0,0.06)\'; this.style.transform=\'translateY(0)\'">' +
                                ...
                                '<div style="display: flex; gap: 10px; margin-top: 15px; justify-content: flex-end;">' +
                                    (donation.hasReview ?
                                        ...
                                        (canWriteReview(donation.createdAt) ?
                                    ...
                                    (donation.paymentStatus === 'REFUNDED' ?
                                        ...
                                        (donation.refundAmount && donation.refundAmount > 0 && !donation.refundedAt ?
                                            '<span style="padding: 8px 16px; background: #f39c12; color: white; border-radius: 6px; font-size: 13px; display: flex; align-items: center; gap: 6px;"><i class="fas fa-clock"></i>환불 승인 대기중</span>' :
                                            (canRefund(donation.createdAt) && donation.paymentStatus === 'COMPLETED' ?
        ...
        async function loadNotifications(skipGenerate = false) {
            try {
                ...
                console.log('로컬 알림 개수:', localNotifications.length);
                ...
                try {
                    ...
                    if (result.success && result.data) {
                        serverNotifications = result.data;
                        console.log('서버 알림 개수:', serverNotifications.length);
                    }
                } catch (serverError) {
                    console.log('서버 알림 로드 실패:', serverError);
                }

                // 4. 서버 알림과 로컬 알림 병합 (중복 제거)
                const allNotifications = [...serverNotifications];
                const serverIds = new Set(serverNotifications.map(n => n.notification_id || n.notificationId));

                localNotifications.forEach(localN => {
                    // 로컬 알림 ID가 서버에 없으면 추가
                    if (!serverIds.has(localN.notification_id)) {
                        allNotifications.push(localN);
                    }
                });

                console.log('병합된 전체 알림 개수:', allNotifications.length);
                ...
                window.cachedNotifications = allNotifications;
                displayNotifications(allNotifications, currentNotificationFilter || 'all');
        ...
        function displayUpcomingEvents() {
            ...
            // window.userEvents 참조
            const userEvents = window.userEvents || {};
        ...
        function deleteEventFromNotification(eventId) {
            ...
            for (const dateStr in userEvents) {
                window.userEvents[dateStr] = userEvents[dateStr].filter(event => event.id !== eventId);

                // 빈 배열이면 날짜 키도 삭제
                if (window.userEvents[dateStr].length === 0) {
                    delete window.userEvents[dateStr];
                }
            }
        ...
        // 리마인더 상태 초기화 (테스트용) - 콘솔에서 resetReminders() 호출
        window.resetReminders = function() {
            const keysToRemove = [];
            for (let i = 0; i < localStorage.length; i++) {
                const key = localStorage.key(i);
                if (key && (key.startsWith('event_reminder_') || key.startsWith('volunteer_reminder_'))) {
                    keysToRemove.push(key);
                }
            }
            keysToRemove.forEach(key => localStorage.removeItem(key));
            console.log('✅ ' + keysToRemove.length + '개의 리마인더 상태가 초기화되었습니다.');
            console.log('페이지를 새로고침하면 알림이 다시 생성됩니다.');
        };
        ...
        // 리뷰 작성 가능 여부 확인 (3일 이내)
        function canWriteReview(createdAt) {
            const createdDate = new Date(createdAt);
            const now = new Date();
            const daysDiff = Math.floor((now - createdDate) / (1000 * 60 * 60 * 24));
            return daysDiff <= 3;
        ...
        function requestRefund(donationId, amount, createdAt) {
            ...
            .then(data => {
                if (data.success) {
                    alert('환불 신청이 접수되었습니다.\n\n예상 환불금액: ' + data.refundAmount.toLocaleString() + '원\n\n영업일 기준 1~2일 소요됩니다.');
