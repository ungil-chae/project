...
public class WelfareService {
    ...
    /**
     * 인기 복지 서비스 조회 (조회수 상위 10개)
     */
    public List<Map<String, Object>> getPopularWelfareServices() {
        try {
            System.out.println("인기 복지 서비스 조회 시작");
            List<Map<String, Object>> allServices = new ArrayList<>();
            
            // 기본 사용자 데이터로 API 호출
            Map<String, String> dummyUser = new HashMap<>();
            dummyUser.put("birthdate", "1990-01-01");
            dummyUser.put("gender", "male");
            dummyUser.put("household_size", "1");
            dummyUser.put("income", "200_300");
            
            // 중앙부처 서비스 조회
            try {
                Map<String, String> apiParams = convertUserToAPIParams(dummyUser);
                List<Map<String, Object>> centralServices = getCentralWelfareServices(dummyUser, apiParams);
                for (Map<String, Object> service : centralServices) {
                    service.put("source", "중앙부처");
                    allServices.add(service);
                }
                System.out.println("중앙부처 서비스 " + centralServices.size() + "개 조회");
            } catch (Exception e) {
                System.err.println("중앙부처 서비스 조회 실패: " + e.getMessage());
            }
            
            // 지자체 서비스 조회
            try {
                dummyUser.put("sido", "서울특별시");
                dummyUser.put("sigungu", "강남구");
                Map<String, String> apiParams = convertUserToAPIParams(dummyUser);
                List<Map<String, Object>> localServices = getLocalWelfareServices(dummyUser, apiParams);
                for (Map<String, Object> service : localServices) {
                    service.put("source", "지자체");
                    allServices.add(service);
                }
                System.out.println("지자체 서비스 " + localServices.size() + "개 조회");
            } catch (Exception e) {
                System.err.println("지자체 서비스 조회 실패: " + e.getMessage());
            }
            
            System.out.println("전체 서비스 " + allServices.size() + "개 조회됨");
            ...
            System.out.println("상위 " + result.size() + "개 서비스 반환");
            return result;
                    
        } catch (Exception e) {
            System.err.println("인기 복지 서비스 조회 실패: " + e.getMessage());
            e.printStackTrace();
            return new ArrayList<>();
        }
    }
